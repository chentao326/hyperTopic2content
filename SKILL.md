...
**B1 网页演示 → topic2content skill**（React 项目 + 分段音频 + 点击翻页）
**B2 MP4 视频 → video-pipeline skill**（从 topic2content React 项目截图 + ffmpeg 拼接）

### 路由规则

| 用户说 | 路由目标 | 输入 |
|--------|---------|------|
| 「出视频」「出 MP4 短视频」 | video-pipeline | Phase 3 公众号文章 |
| 「做演示」「出网页演示」 | topic2content | Phase 3 公众号文章 |
| 「出全套」（含视频） | topic2content 先跑 B1 → video-pipeline 再吃 B1 产物出 B2 | Phase 3 + Phase 2 报告 |
| 「把这个报告/资料做成视频」 | video-pipeline（素材直达） | 用户提供 |

### 传递给 video-pipeline 的内容

- Phase 2 横纵分析报告 .md → 报告类型（DESIGN.md 模板匹配）
- Phase 3 公众号文章 .md → 口播稿提炼输入
- topic2content 产出的 React 项目路径 → B2 截图素材

### 传递给 topic2content 的内容

- Phase 3 公众号文章 .md → 口播稿提炼
- Phase 2 横纵分析报告 .md → 背景素材
...
v3.1 — 2026-06-26 **第二轮 grilling 修正**：Phase 4 路由澄清 — B1 网页演示路由到 topic2content（web-video-presentation 专家），B2 MP4 路由到 video-pipeline（截图合成专家）。B1 是 B2 输入源，不是解耦关系。

v3.0 — 2026-06-26 **架构重构**：Phase 4（视频生成）拆为独立 `video-pipeline` skill。hyperTopic2content 专注 Phase 0→3→5→6，视频需求路由到 video-pipeline。
