...
1. **确认PDF转换脚本可用**：本Skill自带 `scripts/md_to_pdf.py`（基于WeasyPrint），用于将最终Markdown报告转为排版精美的PDF。确保依赖已安装。macOS 用户如遇 PDF 生成失败，参考 `references/macos-weasyprint-setup.md`。
...
⚠️ **重要提示：delegate_task 子Agent 搜索能力受限。** 子Agent 通常无法使用 terminal/curl，browser 工具也依赖 Camofox 服务（localhost:9377）等未启动的后端。**不要依赖子Agent做联网搜索**——它们很可能返回「工具不可用」并仅提供训练数据（无来源URL，信息可能过时）。如果一定要用子Agent，先检查 Camofox/Open Design 等服务是否在运行。

**推荐的主流程**（本Agent直接执行搜索，按优先级排列）：

1. **Playwright 抓取正文（首选）**：用户机器上 `pw` wrapper 已装好可用（`~/.hermes/bin/pw scrape <url>`），系统 Python 3.9 的 Playwright 已配置 Chromium。对中文平台（36氪、头条、CSDN、AI产品库、什么值得买、B站）效果稳定。知乎仍可能 403。用法：
   ```bash
   pw scrape https://example.com/article
   ```
   输出 JSON 格式，包含标题、正文、时间。被拦的网站不要反复重试，换 DDGS 兜底。

2. **DDGS 搜索**（找 URL 列表用）：先用 DDGS 搜到相关文章的 URL，再用 Playwright 抓正文。
   ```bash
   cd ~/.hermes/hermes-agent && venv/bin/python3 /tmp/search_xxx.py
   ```
   脚本用 write_file 写入 /tmp 再执行，避免 shell 编码破坏中文。DDGS 对中文关键词效果不稳定，优先用英文关键词搜中文内容，或简化查询词加 site: 限定。

   语言策略：DDGS 对中文查询支持不稳定，对混合中英文查询效果尤其差。研究中国公司/产品/概念时用中文关键词；研究国际公司（韩国、日本、美国等）时优先用英文关键词，DDGS 返回质量更高。如果英文搜索返回结果偏题（如搜索韩国公司但返回了工具品牌 SK），换中文关键词重试。推荐策略：每轮搜索先用英文搜一轮，再用中文搜一轮做覆盖补充。

3. **curl 抓取**（仅限无 JS 渲染的页面）：对纯文本页（GitHub raw、新闻 API 端点）可用。多数中文站点正文靠 JS 渲染，curl 只能拿元数据摘要。

4. **last30days-cn 辅助**：如果研究对象是近期热门话题，可以用 `last30days-cn` 扫描微博/B站/知乎补充实时讨论热度（微博爬虫模式最稳定，B站/知乎/小红书 API 经常失败）。

5. **最后一次全量扫描**（可选）：报告写完后，调用 `last30days-cn --quick --emit compact` 对研究对象做终扫，确认没有遗漏近期重大事件。

**信息收集的最低标准**：DDGS 搜 3-4 个不同角度（纵向历史、横向竞品、用户口碑、战略背景），每轮 8-10 条结果 -> 对关键 URL 用 Playwright 抓正文。如果某个角度搜索结果太少，换关键词重试，不要跳过。

建议的分工（由本Agent串行或手动并行执行）：
- **搜索 1 — 纵向信息**：研究对象的起源、发展历程、关键事件、版本迭代
- **搜索 2 — 横向信息**：竞品识别、对比评测、用户口碑
- **搜索 3 — 补充信息**：团队背景、商业模式、行业环境
- **搜索 4 — 战略/生态**：母公司战略、产品矩阵关系
...
| 行业分析 | 权威媒体原创报道（非转载）。亚洲市场：Yonhap News、The Korea Times、Chosun Ilbo（韩国）；日经亚洲、Nikkei（日本）；Reuters Asia、Bloomberg Asia |
...
研究中如需引用行业权威数据（市场规模、市场份额、关键事件时间线），先查本库的 `references/` 目录下有没有现成的行业数据参考文件。例如研究半导体/AI 芯片时，`references/semiconductor-industry-data-2026.md` 已预置 Gartner/IDC/Deloitte 等多家机构的 2026 年市场预测、HBM 三强份额分布和关键事件清单。
...
2. **安装依赖**（如未安装）：
   ...
   # 安装 Python 包（注意用哪个 Python）
   pip install weasyprint markdown --break-system-packages
   ```
3. **运行转换脚本**：
   ```bash
   # macOS: 需要设置 DYLD_LIBRARY_PATH 指向 homebrew 库
   DYLD_LIBRARY_PATH=/opt/homebrew/lib python3 [skill目录]/scripts/md_to_pdf.py input.md output.pdf --title "标题" --author "作者"
   ```
4. 脚本会自动生成中间HTML文件（便于调试和浏览器查看）和最终PDF

### WeasyPrint / macOS 常见问题

| 症状 | 原因 | 解决 |
|------|------|------|
| `cannot load library 'libgobject-2.0-0'` | 缺系统库 | `brew install pango gobject-introspection` |
| 装完 brew 库仍然报同样错误 | 用了系统 Python 3.9 的旧版 weasyprint（v66），不识 macOS `.dylib` | 用 Hermes 运行时自带的 Python 3.12 + weasyprint v69+：`/Users/chentao/.hermes-web-ui/desktop-runtime/hermes/0.16.0/mac-arm64/python/bin/python3` |
| `pip install weasyprint` 装到了错误的 Python | `pip` 命令指向 Hermes 运行时而非系统 Python | 先 `which pip` 确认路径；如需装到系统 Python 用 `pip3 install` |
| WeasyPrint 报 HTML 中的图片路径无法访问 | 相对路径问题 | Markdown 中的图片使用绝对路径 |
...
如果用户要求基于报告写公众号文章，参考 `references/research-to-article-pipeline.md` 中的完整流水线。核心要点：不是缩写报告，而是从横纵交汇洞察中提取最有冲击力的判断，用口语化风格重新讲一遍。
