...
在跑L1-L4风格自检之前，先做一轮独立的逻辑审核。风格自检管的是「像不像真人写的」，逻辑审核管的是「说的对不对」。两者检查的东西完全不同，不能互相替代。

### 逻辑审核清单

**结论与证据一致性**：回看文章中每个核心结论，检查它是否被前文的证据完全支撑。如果前面列举了A、B、C三个优势，结论却说「跟A无关，只跟B有关」，那就是以偏概全。修复方式：要么结论改成「B是一方面，更关键的是C」这种更精确的表述，要么删掉前面不支持结论的论据。

**核心论点重复检查**：长文（4000字以上）写到后半段，很容易不自觉地重复前文已经展开过的论点。这不是「回环呼应」（刻意的callback），而是忘了自己写过。检查方法：把文章的核心判断句摘出来，看有没有两句话说的是同一件事。如果发现重复，后一处要么删掉，要么改成「前面聊了这么多，归根结底就是这个判断」这种引回式表述，让它变成有意的总结而不是无意的重复。

**成语和固定搭配**：口语化写作不意味着可以用错成语。「绑绑有余」不是成语，正确写法是「绰绰有余」。「首当其冲」不是「首先」的意思。「差强人意」是「还算满意」不是「不满意」。写完后扫一遍文中的成语和固定搭配，不确定的查一下。

**跨段落事实一致性**：如果前面说「2024年5月上线」，后面又说「上线一年多」，确认写作时间点是否对得上。如果前面说即梦「免费」，后面又说「充了月卡会员」，需要交代清楚免费和付费的区别，不能让读者觉得前后矛盾。

**逻辑审核输出**：列出发现的问题及修复方式。修复优先级：事实错误 > 以偏概全 > 论点重复 > 成语错误。
